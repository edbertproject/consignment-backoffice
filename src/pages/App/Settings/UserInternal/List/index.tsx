...
const breadcrumbs = [
  ...
  {
    label: "User Internal",
    link: "/pengaturan/user-internal",
  },
];

const AppSettingsUserInternalList: React.FC<any> = () => {
  ...
  const getTableData = async (reset: boolean = false) => {
    ...
    await UserInternalRepository.all(params)
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <Space size="middle">
          ...
          <Accessible access="delete user internal">
            <Text
  ...
  const deleteModalCallback = async () => {
    if (selectedDeleteId.get()) {
      await UserInternalRepository.delete(selectedDeleteId.get())
        .then((res: AxiosResponse) => {
  ...
  return (
    <>
      <WuiContainer>
        <WuiSectionTitle title={"User Internal"} breadcrumbs={breadcrumbs}>
...
export default AppSettingsUserInternalList;
