...
const AppSettingsRoleList: React.FC<any> = () => {
  ...
  const getTableData = async (reset: boolean = false) => {
    ...
    await RoleRepository.all(params)
  ...
  const deleteModalCallback = async () => {
    if (selectedDeleteId.get()) {
      await RoleRepository.delete(selectedDeleteId.get())
        .then((res: AxiosResponse) => {
          showDeleteModal.set(false);
          selectedDeleteId.set(0);
          openNotification(
            "success",
            t("notification.success.deleteItem", { item: "Role" })
          );
          getTableData(true);
        })
        .catch((e: AxiosError) => {
  ...
  return (
    <>
      <WuiContainer>
        <WuiSectionTitle title={"Role"} breadcrumbs={breadcrumbs}>
...
export default AppSettingsRoleList;
