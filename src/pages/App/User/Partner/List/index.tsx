...
import { PARTNER_TAG_COLOR } from "../../../../../constant";
...
const AppUserPartnerList: React.FC<any> = () => {
  ...
  const getTableData = async (reset: boolean = false) => {
    ...
    await UserPartnerRepository.all(params)
  ...
  const columns: any = [
    ...
    },
    {
      ...
      render: (status: any) => {
        return (
          <Tag color={PARTNER_TAG_COLOR[status]} key={status}>
            {status}
          </Tag>
        );
      },
    },
    {
      ...
      render: (text: any, record: any) => (
        <Space size="middle">
          ...
          <Accessible access="delete user partner">
            <Text
  ...
  const deleteModalCallback = async () => {
    if (selectedDeleteId.get()) {
      await UserPartnerRepository.delete(selectedDeleteId.get())
        .then((res: AxiosResponse) => {
...
export default AppUserPartnerList;
